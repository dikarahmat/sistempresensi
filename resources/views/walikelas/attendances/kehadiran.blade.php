...
@section('page_subtitle', 'Lihat histori kehadiran per kelas binaan')
                ...
                <tbody>
                    ...
                    <tr class="{{ $loop->odd ? 'baris-abu' : 'baris-putih' }}">
                        ...
                        <td class="text-center text-nowrap">
                            <div class="d-inline-flex align-items-center justify-content-center gap-1.5 flex-nowrap">
                                {{-- Tombol Presensi --}}
                                <a href="{{ route('guru.absensi.index', ['class_id' => $item->id]) }}" class="btn-action-presensi" title="Buka Presensi Hari Ini {{ $shortName }}">
                                    <i class='bx bx-qr-scan fs-6'></i>
                                    <span>Presensi</span>
                                </a>

                                {{-- Tombol Lihat --}}
                                <a href="{{ route('guru.absensi.show', $item->id) }}" class="btn-action-lihat" title="Lihat Rekap Histori {{ $shortName }}">
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-5 text-muted fw-normal">
                            ...
                            Belum ada data rombel yang dibina oleh akun Anda.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
